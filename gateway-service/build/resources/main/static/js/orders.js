...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 저장된 장바구니가 있는지 확인하고 복구하기
    const savedCart = localStorage.getItem('savedCart');
    if (savedCart) {
        // 사용자에게 물어보고 복구
        if(confirm("이전에 담아둔 장바구니 내역이 있습니다. 불러오시겠습니까?")) {
            cart = JSON.parse(savedCart); // 저장된 문자열을 다시 배열로 변환
            renderCart();
        } else {
            localStorage.removeItem('savedCart'); // 필요 없다고 하면 삭제
        }
    }
...
async function handlePlaceOrder() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            localStorage.removeItem('savedCart'); // 임시 저장소 청소
